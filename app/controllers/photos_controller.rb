class PhotosController < ApplicationController
  ...
  def comment
    ...
    # Assuming that the_id is the ID of the Photo for which the comment is being made.
    a_new_comment = Comment.new
    a_new_comment.photo_id = the_id
    a_new_comment.author_id = input_author
    a_new_comment.body = input_comment
  
    if a_new_comment.save
      redirect_to("/photos/" + the_id.to_s)
    else
      # Handle the error, perhaps by redirecting to a different page or showing an error message
    end
  end
end
